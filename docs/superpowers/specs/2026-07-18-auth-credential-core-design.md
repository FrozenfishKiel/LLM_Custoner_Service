...
7. Hash and verify are async public operations. Argon2 runs through AnyIO's worker-thread bridge with a dedicated capacity limiter of four concurrent jobs, so application event loops never execute Argon2 directly. A second admission limiter allows at most 20 admitted jobs including running work; excess attempts fail immediately with `PasswordHashingOverloaded("Password hashing capacity is unavailable")`. Later per-IP/account rate limiting remains mandatory.
8. Before verifying an encoded hash, the module parses its parameters without performing Argon2 work. Only Argon2id version 19 hashes within memory 8-65536 KiB, time cost 1-3, parallelism 1-4, hash length 16-64, and salt length 8-32 are eligible. Higher-cost or malformed attacker-controlled encodings take the bounded dummy path instead of allocating their declared resources.
